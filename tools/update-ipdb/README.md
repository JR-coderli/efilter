...
用于定时下载并原子替换 IP2Location / IP2Proxy 的 BIN 数据库文件。
...
编辑 `.env.production.example` 中的 token 和下载 URL，或设置环境变量：
...
更新完成后，需要重启 `risk-engine` 以加载新的 BIN 文件：

```bash
systemctl restart risk-engine
# 或
kill -HUP <pid>
# 或
